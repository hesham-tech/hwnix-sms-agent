...
class SessionManager(context: Context) {
    ...
    companion object {
        ...
        fun sanitizeUrl(url: String?): String {
            if (url.isNullOrEmpty()) return DEFAULT_BASE_URL
            var cleanUrl = url.trim()
            if (cleanUrl.contains("10.0.2.2") || cleanUrl.contains("localhost")) {
                cleanUrl = "https://bill-api.hwnix.com/api"
            }
    ...
    fun getBaseUrl(): String {
        val url = sharedPreferences.getString(KEY_BASE_URL, DEFAULT_BASE_URL) ?: DEFAULT_BASE_URL
